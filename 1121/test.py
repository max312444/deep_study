...
# Flask 애플리케이션 객체 생성
# 이 객체에 라우트, 훅(before/after/teardown), 설정 등이 모두 등록됨
app = Flask(__name__)
...
# 2. "/" 경로(GET) 요청 처리
# -----------------------------
@app.route("/", methods=["GET"])  # "/" URL에 GET 요청이 오면 아래 함수 실행
def home():
...
# -----------------------------
@app.after_request  # View 함수 실행 후, 응답을 보내기 직전에 실행되는 데코레이터
def prt_after(rsp: Response):
    # 응답 헤더 중 "GSC" 값 출력
    # "/" 경로에서는 "gsc", 다른 경로에서는 None
    print(rsp.headers.get("GSC"))

    # after_request는 반드시 Response 객체를 return해야 함
    return rsp


# -----------------------------
# 5. 요청 처리가 완전히 끝난 후 항상 실행되는 teardown Hook
# -----------------------------
@app.teardown_request  # 정상/에러 상관없이 요청 종료 후 실행되는 cleanup 단계
def teardown(e):
    # e는 예외 객체이며, 에러가 없으면 None
    # DB 세션 종료, 리소스 정리 등의 용도로 사용
    print(e)
